...
if(isset($_POST['newUserForm'])) {
    $readCurrentData = $db->query("SELECT * FROM veriler WHERE kullanici_id = '".$_POST['kullanici_id']."'")->fetch();
    if(count($readCurrentData)>1) {
        echo "Böyle bir kullanıcı bulunmakta.";
        header("Location: index.php");
    } else {
        $newUserInsert = $db->prepare("INSERT INTO veriler SET kullanici_id=?, Firma_Adi = ?, Yönetici_Ad_Soyad = ?, Telefon = ?, Mail = ?, Adres= ?");
        $newUserInsert->execute(array($_POST['kullanici_id'],$_POST['Firma_Adi'], $_POST['Yönetici_Ad_Soyad'], $_POST['Telefon'], $_POST['Mail'], $_POST['Adres']));
        header("Location: index.php");
    }
}
